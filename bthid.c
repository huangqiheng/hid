...
static unsigned int de_get_uint(uint8_t *de) {
    ...
    switch (size) {
        case DE_SIZE_8:
            return de[pos];
        case DE_SIZE_16:
            return big_endian_read_16(de, pos);
        case DE_SIZE_32:
            return big_endian_read_32(de, pos);
        default:
...
// bthid_devs and associated btstack_util {{{
btstack_linked_list_t bthid_devs = NULL;

static bthid_dev_t * finddev_addr(bd_addr_t addr) {
    btstack_linked_list_iterator_t it;
    btstack_linked_list_iterator_init(&it, &bthid_devs);
    while (btstack_linked_list_iterator_has_next(&it)) {
        bthid_dev_t *dev = (bthid_dev_t *)btstack_linked_list_iterator_next(&it);
...
static void sdp_packet_handler(uint8_t *packet, int size) {
    ...
    int attr = little_endian_read_16(packet, 3);
    uint8_t *de = packet + 7;
    int de_size = size - 7;
...
void bthid_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size) {
    ...
    if (packet_type == HCI_EVENT_PACKET &&
        packet[0] == SDP_EVENT_QUERY_COMPLETE) {
        dev = sdp_query_dev;
        sdp_query_dev = NULL;
        pump_attributes(dev);
    }
    ...
    switch (packet[0]) {
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            dev = finddev_handle(little_endian_read_16(packet, 3));
            if (dev) {
                printf("Disconnected\n");
                uhid_unregister(dev);
                deletedev(dev);
