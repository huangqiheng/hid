...
btstack_linked_list_t seen_devs = NULL;

int have_seen(bd_addr_t addr) {
    btstack_linked_list_iterator_t it;
    btstack_linked_list_iterator_init(&it, &seen_devs);
    while (btstack_linked_list_iterator_has_next(&it)) {
        seen_dev_t *dev = (seen_dev_t *)btstack_linked_list_iterator_next(&it);
    ...
    switch (packet[0]) {
        ...
        case HCI_EVENT_CONNECTION_COMPLETE:
            ...
            if (packet[2]) { // failure to establish HCI connection
                ...
                remote_handle = little_endian_read_16(packet, 3);
            }
            break;
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            if (remote_handle == little_endian_read_16(packet, 3) &&
                paired) {
                bt_send_cmd(&hci_create_connection, &remote, 0x0000, 0, 0, 0, 0);  // XXX we have no way to find valid packet types
